...
begin
  ...
  dxButtonCambiarImg2.Enabled := Estado;
...
begin
  try
    ...
    try
      Screen.Cursor := crAppStart;
      zOtros.FieldByName('ContrasenaUpt').AsString := cxTextOContrasena.Text;
      zOtros.Post;
...
begin
 try
   ...
   try
     Screen.Cursor := crAppStart;
     YaLeido := False;
     zUsuario.FieldByName('ContrasenaUpt').AsString := cxTextContrasena.Text;
     zUsuario.Post;
...
begin
  YaLeido := False;
  ocultarOtros;
  if not AsignarSQL(zOtros, 'master_usuarios', pReadOnly) then
    raise Exception.Create(pErrorConsulta + '[' + 'Master_usuarios' + ']');
